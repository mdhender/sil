...
`PLAN.md` holds the milestone table and is the authority on what is done. As of M5 a SIL program goes from source through the assembler into the machine and runs. Twelve of the 119 operations are implemented; the rest fault with their own name.
...
- `pkg/sil` — the machine. `Cell` and `Core` (`cell.go`), `VM`/`Step`/`Run` (`vm.go`), the `Host` boundary (`host.go`), and one method per operation (`macros.go`).
